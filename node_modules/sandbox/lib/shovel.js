...
function getSafeRunner() {
  ...
  return function run(comm, src) {
    ...
    global.$$_SUCCESS__ = null
    var result = UserScript(src)();
    send('successid', global.$$_SUCCESS__)
    send('end', result);
  }
}
...
function run() {
  ...
  try {
    safeRunner({
      send: function (event, value) {
        ...
        switch (event) {
          case 'stdout':
            var v = JSON.parse(value)[0]
            console.push(v ? v.toString() : v);
            break;

          case 'successid':
             successId = JSON.parse(value)[0];
             break;
  ...
  var output = JSON.stringify( { result: util.inspect( result ), console: console, error: exception, success_id: successId } )
